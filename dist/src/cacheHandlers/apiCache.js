...
import { buildDownstreamHeaders, buildUpstreamHeaders, cachePutObservabilityHeaders, cacheGetObservabilityHeaders, } from '../util/headers.js';
import { classifyRequest, headerToCacheTags, isInvalidated } from '../util/cache.js';
import { METHODS_WITH_BODY, NO_BODY_RESPONSES, CACHE_CONFIG, resolveConfiguredOrigin, resolveOriginAuthHeader, } from '../constants/index.js';
...
const readNodeRequestBody = async (nodeRequest) => {
    if (!nodeRequest || nodeRequest.readableEnded)
        return Buffer.alloc(0);
    return await new Promise((resolve, reject) => {
        const chunks = [];
        nodeRequest.on('error', reject);
        nodeRequest.on('data', (chunk) => chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk)));
        nodeRequest.on('end', () => resolve(chunks.length ? Buffer.concat(chunks) : Buffer.alloc(0)));
    });
};
const readRequestBody = async (request) => {
    const nodeBody = await readNodeRequestBody(request._nodeRequest);
...
const fetchFromCache = async (request, url, upstreamHeaders, cacheInvalidations, startTime) => {
    ...
    if (shouldCache) {
        ...
        const headersForCache = Object.fromEntries(Array.from(downstreamHeaders.entries()).filter(([k]) => k.toLowerCase() !== 'set-cookie'));
        if (bodyBuf.length) {
...
export const handleAPI = async (request, cacheInvalidations) => {
    const startTime = performance.now();
    const origin = resolveConfiguredOrigin('apiOrigin');
    let url = new URL(request.url, origin);
    if (CACHE_CONFIG.apiPathReplacement) {
        url = new URL(request.url.replace(CACHE_CONFIG.apiPathReplacement.search, CACHE_CONFIG.apiPathReplacement.replace), origin);
    }
