...
const handleDeletes = async (table, queryAttr, op, queryVal) => {
...
const handleCacheTagRecordDeletion = async (cacheTag) => {
    if (!cacheTag)
        return [400, 'cacheTag must not be empty'];
...
const handleUrlRecordDeletion = async (url) => {
    if (!url)
        return [400, 'url must not be empty'];
