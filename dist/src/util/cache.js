 ...
 */
export const headerToCacheTags = (headers) => headers.get(CACHE_CONFIG.cacheTagsHeader) || headers.get(CACHE_CONFIG.cacheTagsHeader?.toLowerCase()) || '';
/**
 ...
 */
export const isInvalidated = (type, invalidationMap, refreshedAt, groupCode) => {
 ...
 */
const parseDuration = (ttl) => {
    const s = String(ttl || '')
        .trim()
        .toLowerCase();
    if (s === SPECIAL_TTL.NEVER)
        return { mode: SPECIAL_TTL.NEVER, seconds: 0 };
    if (s === SPECIAL_TTL.ORIGIN)
        return { mode: SPECIAL_TTL.ORIGIN, seconds: 0 };
    ...
    const secs = n * multiplicationFactor[unit];
 ...
 * @returns {string|null}
 */
const fixedPrefixFromRegexStr = (regexStr) => {
 ...
 * @returns {number}
 */
const specificityScore = (regexStr, hasConds) => {
...
const normalizeConds = (rule) => {
    ...
    (rule.additionalMatchCritera ?? []).forEach((criterion) => {
        ...
        const needsValues = op === CONDITION_OPERATOR.EQUALS ||
...
const evalCond = (cond, req) => {
    ...
    if (cond.type === MATCH_TYPE.HEADER) {
        const headerVal = headerGet(req.headers, cond.key);
 ...
 */
const applyOperator = (op, received, searchList) => {
    ...
    switch (op) {
        ...
        case CONDITION_OPERATOR.NOT_EQUALS: {
            ...
            if (!gotExists)
                return true; // no value present => not equal
            return received !== searchList[0];
        }
 ...
 */
const buildRuleIndex = (ttlRulesRows) => {
    const rules = [];
    for (const row of ttlRulesRows || []) {
        ...
        for (const pat of patterns) {
            rules.push({
                id: row.id || '',
                name: row.description || row.id || 'rule',
                patternText: pat,
    ...
    for (const arr of buckets.values())
        arr.sort((a, b) => b.specificity - a.specificity);
...
export const classifyRequest = (req, cacheKey) => {
    ...
    for (const r of candidates) {
        ...
        if (r.conds.length && r.conds.some((cond) => !evalCond(cond, req)))
            continue;
...
const applySettings = async () => {
    ...
    const rules = (await Array.fromAsync(rulesIter));
    swapRules(rules);
    clearMemo();
};
